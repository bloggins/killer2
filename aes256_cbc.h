/*
	aes256_cbc.h - self-contained AES-256-CBC (encrypt/decrypt) for the loader.
	Public-domain style implementation (tiny-AES layout), no external crypto deps.
*/
#ifndef AES256_CBC_H
#define AES256_CBC_H

#include <stdint.h>
#include <string.h>

#define AES_BLOCKLEN    16
#define AES256_KEYLEN   32
#define AES_keyExpSize  240
#define AES_Nb          4
#define AES_Nk          8
#define AES_Nr          14

static const uint8_t sbox[256] = {
	
};

static const uint8_t rsbox[256] = {
	
};

static const uint8_t Rcon[11] = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };

static uint8_t xtime(uint8_t x) {
	return (uint8_t)((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}

static uint8_t Multiply(uint8_t x, uint8_t y) {
	return (uint8_t)(((y & 1) * x) ^ ((y >> 1 & 1) * xtime(x)) ^
	                 ((y >> 2 & 1) * xtime(xtime(x))) ^
	                 ((y >> 3 & 1) * xtime(xtime(xtime(x)))) ^
	                 ((y >> 4 & 1) * xtime(xtime(xtime(xtime(x))))));
}

static void KeyExpansion(uint8_t* RoundKey, const uint8_t* Key) {
	unsigned i, j, k;
	uint8_t tempa[4];
	for (i = 0; i < AES_Nk; ++i) {
		RoundKey[(i * 4) + 0] = Key[(i * 4) + 0];
		RoundKey[(i * 4) + 1] = Key[(i * 4) + 1];
		RoundKey[(i * 4) + 2] = Key[(i * 4) + 2];
		RoundKey[(i * 4) + 3] = Key[(i * 4) + 3];
	}
	for (i = AES_Nk; i < AES_Nb * (AES_Nr + 1); ++i) {
		k = (i - 1) * 4;
		tempa[0] = RoundKey[k + 0];
		tempa[1] = RoundKey[k + 1];
		tempa[2] = RoundKey[k + 2];
		tempa[3] = RoundKey[k + 3];
		if (i % AES_Nk == 0) {
			const uint8_t u8tmp = tempa[0];
			tempa[0] = tempa[1];
			tempa[1] = tempa[2];
			tempa[2] = tempa[3];
			tempa[3] = u8tmp;
			tempa[0] = sbox[tempa[0]];
			tempa[1] = sbox[tempa[1]];
			tempa[2] = sbox[tempa[2]];
			tempa[3] = sbox[tempa[3]];
			tempa[0] = tempa[0] ^ Rcon[i / AES_Nk];
		}
		if (i % AES_Nk == 4) {
			tempa[0] = sbox[tempa[0]];
			tempa[1] = sbox[tempa[1]];
			tempa[2] = sbox[tempa[2]];
			tempa[3] = sbox[tempa[3]];
		}
		j = i * 4; k = (i - AES_Nk) * 4;
		RoundKey[j + 0] = RoundKey[k + 0] ^ tempa[0];
		RoundKey[j + 1] = RoundKey[k + 1] ^ tempa[1];
		RoundKey[j + 2] = RoundKey[k + 2] ^ tempa[2];
		RoundKey[j + 3] = RoundKey[k + 3] ^ tempa[3];
	}
}

static void AddRoundKey(uint8_t round, uint8_t* state, const uint8_t* RoundKey) {
	for (uint8_t i = 0; i < 16; ++i) {
		state[i] ^= RoundKey[round * 16 + i];
	}
}

static void SubBytes(uint8_t* state) {
	for (uint8_t i = 0; i < 16; ++i) {
		state[i] = sbox[state[i]];
	}
}

static void ShiftRows(uint8_t* state) {
	uint8_t tmp;
	tmp = state[1];  state[1] = state[5];  state[5] = state[9];  state[9] = state[13];  state[13] = tmp;
	tmp = state[2];  state[2] = state[10];  state[10] = tmp;
	tmp = state[6];  state[6] = state[14];  state[14] = tmp;
	tmp = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3];   state[3] = tmp;
}

static void MixColumns(uint8_t* state) {
	uint8_t Tmp, Tm, t;
	for (uint8_t i = 0; i < 16; i += 4) {
		t = state[i];
		Tmp = state[i] ^ state[i + 1] ^ state[i + 2] ^ state[i + 3];
		Tm = state[i] ^ state[i + 1];       Tm = xtime(Tm);       state[i] ^= Tm ^ Tmp;
		Tm = state[i + 1] ^ state[i + 2];   Tm = xtime(Tm);       state[i + 1] ^= Tm ^ Tmp;
		Tm = state[i + 2] ^ state[i + 3];   Tm = xtime(Tm);       state[i + 2] ^= Tm ^ Tmp;
		Tm = state[i + 3] ^ t;              Tm = xtime(Tm);       state[i + 3] ^= Tm ^ Tmp;
	}
}

static void Cipher(uint8_t* state, const uint8_t* RoundKey) {
	uint8_t round = 0;
	AddRoundKey(0, state, RoundKey);
	for (round = 1; round < AES_Nr; ++round) {
		SubBytes(state);
		ShiftRows(state);
		MixColumns(state);
		AddRoundKey(round, state, RoundKey);
	}
	SubBytes(state);
	ShiftRows(state);
	AddRoundKey(AES_Nr, state, RoundKey);
}

static void InvMixColumns(uint8_t* state) {
	uint8_t a, b, c, d;
	for (uint8_t i = 0; i < 16; i += 4) {
		a = state[i]; b = state[i + 1]; c = state[i + 2]; d = state[i + 3];
		state[i]     = (uint8_t)(Multiply(a, 0x0e) ^ Multiply(b, 0x0b) ^ Multiply(c, 0x0d) ^ Multiply(d, 0x09));
		state[i + 1] = (uint8_t)(Multiply(a, 0x09) ^ Multiply(b, 0x0e) ^ Multiply(c, 0x0b) ^ Multiply(d, 0x0d));
		state[i + 2] = (uint8_t)(Multiply(a, 0x0d) ^ Multiply(b, 0x09) ^ Multiply(c, 0x0e) ^ Multiply(d, 0x0b));
		state[i + 3] = (uint8_t)(Multiply(a, 0x0b) ^ Multiply(b, 0x0d) ^ Multiply(c, 0x09) ^ Multiply(d, 0x0e));
	}
}

static void InvSubBytes(uint8_t* state) {
	for (uint8_t i = 0; i < 16; ++i) {
		state[i] = rsbox[state[i]];
	}
}

static void InvShiftRows(uint8_t* state) {
	uint8_t tmp;
	tmp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = tmp;
	tmp = state[2];  state[2] = state[10]; state[10] = tmp;
	tmp = state[6];  state[6] = state[14]; state[14] = tmp;
	tmp = state[3];  state[3] = state[7];  state[7] = state[11]; state[11] = state[15]; state[15] = tmp;
}

static void InvCipher(uint8_t* state, const uint8_t* RoundKey) {
	uint8_t round = 0;
	AddRoundKey(AES_Nr, state, RoundKey);
	for (round = (uint8_t)(AES_Nr - 1); round > 0; --round) {
		InvShiftRows(state);
		InvSubBytes(state);
		AddRoundKey(round, state, RoundKey);
		InvMixColumns(state);
	}
	InvShiftRows(state);
	InvSubBytes(state);
	AddRoundKey(0, state, RoundKey);
}

/* in-place CBC encrypt/decrypt; length must be a multiple of 16 */
static void AES256_CBC_encrypt_buffer(uint8_t* buf, uint32_t length, const uint8_t* key, const uint8_t* iv) {
	uint8_t RoundKey[AES_keyExpSize];
	uint8_t iv_copy[AES_BLOCKLEN];
	KeyExpansion(RoundKey, key);
	memcpy(iv_copy, iv, AES_BLOCKLEN);
	for (uint32_t i = 0; i < length; i += AES_BLOCKLEN) {
		for (uint8_t j = 0; j < AES_BLOCKLEN; ++j) {
			buf[i + j] ^= iv_copy[j];
		}
		Cipher(buf + i, RoundKey);
		memcpy(iv_copy, buf + i, AES_BLOCKLEN);
	}
}

static void AES256_CBC_decrypt_buffer(uint8_t* buf, uint32_t length, const uint8_t* key, const uint8_t* iv) {
	uint8_t RoundKey[AES_keyExpSize];
	uint8_t iv_copy[AES_BLOCKLEN];
	uint8_t prev_block[AES_BLOCKLEN];
	KeyExpansion(RoundKey, key);
	memcpy(iv_copy, iv, AES_BLOCKLEN);
	for (uint32_t i = 0; i < length; i += AES_BLOCKLEN) {
		memcpy(prev_block, buf + i, AES_BLOCKLEN);
		InvCipher(buf + i, RoundKey);
		for (uint8_t j = 0; j < AES_BLOCKLEN; ++j) {
			buf[i + j] ^= iv_copy[j];
		}
		memcpy(iv_copy, prev_block, AES_BLOCKLEN);
	}
}

#endif /* AES256_CBC_H */
